...
class CoveredCallRequest:
    ...
    def for_ticker(self, ticker: str) -> "CoveredCallRequest":
...
def _invalid_request(
    ...
    *,
    ...
    endpoint: str = "screen_covered_calls",
) -> InvalidRequestError:
...
def _request_params(request: CoveredCallRequest) -> dict[str, object]:
    return {
...
def _planned_cost(request: CoveredCallRequest, expiration: dt.date | str) -> PlanCost:
    if expiration == "*" and request.strike_range is None:
        return "high"
...
def _planned_local_computation(request: CoveredCallRequest, expiration: dt.date | str) -> PlanCost:
    if request.greeks_source != "local":
        return "low"
    if expiration == "*" and request.strike_range is None:
        return "high"
...
def plan_covered_calls(
    request: CoveredCallRequest,
    *,
    conn=None,
) -> ScreenerPlan:
    """Explain the expected covered-call screening cost before fetching data."""
...
def _sort_covered_calls(calls: pl.DataFrame, *, rank_by: str, top_n: int | None) -> pl.DataFrame:
    tie_breakers = [column for column in ["premium", "probability_otm"] if column != rank_by]
    return sort_and_limit(
        calls,
        rank_by=rank_by,
        tie_breakers=tie_breakers,
        descending=[True] * (1 + len(tie_breakers)),
        top_n=top_n,
    )


def _execute_covered_call_request(
    request: CoveredCallRequest,
    *,
    ...
    candidate_rows = len(calls)
    calls = _sort_covered_calls(calls, rank_by=request.rank_by, top_n=request.top_n)
...
def screen_covered_calls(
    ...
    *,
    ...
    """Screen covered calls from a typed request and return diagnostics."""
    return _execute_covered_call_request(request, client=client, conn=conn)
    ...
    *,
    ...
    while attempts < max(retry_policy.max_attempts, 1):
        ...
        try:
            ...
            result = screen_covered_calls(request, client=client, conn=conn)
            if (
                ...
                raise ThetaTimeoutError(
                    "Covered-call screening exceeded the per-ticker timeout.",
                    ticker=request.ticker,
                    endpoint="screen_covered_calls",
                    params=_request_params(request),
                    retryable=True,
                    user_message="Covered-call screening exceeded the per-ticker timeout.",
                )
            return result
        except Exception as exc:
            error = (
...
def screen_covered_call_watchlist(
    tickers: list[str],
    request: CoveredCallRequest,
    client: Client | None = None,
    *,
    ...
    """Screen a watchlist for covered calls with per-ticker failures."""
    ...
    def run_one(ticker: str) -> tuple[str, ScreenerResult | TickerFailure]:
        if circuit_breaker.is_open():
            return ticker, _circuit_open_failure(ticker, endpoint="screen_covered_calls")
        rate_limiter.wait()
...
def _warm_covered_call_request(
    request: CoveredCallRequest,
    *,
    client: Client | None = None,
    timeout_policy: TimeoutPolicy | None = None,
    ...
    started_at = time.perf_counter()
    expiration, warnings = _validate_request(request, endpoint="warm_covered_call_cache")
    ticker = request.ticker or ""
    today = dt.date.today()
    plan = plan_covered_calls(request, conn=conn)
    ...
    if (
        ...
        raise ThetaTimeoutError(
            "Covered-call cache warmup exceeded the per-ticker timeout.",
            ticker=ticker,
            endpoint="warm_covered_call_cache",
            params=_request_params(request),
            retryable=True,
            user_message="Covered-call cache warmup exceeded the per-ticker timeout.",
        )
...
def _attempt_warm(
    ...
    *,
    ...
    while attempts < max(retry_policy.max_attempts, 1):
        attempts += 1
        try:
            return _warm_covered_call_request(
                request,
                client=client,
                timeout_policy=timeout_policy,
                conn=conn,
...
def warm_covered_call_cache(
    ...
    *,
    ...
    """Warm cache inputs for covered-call screening without ranking candidates."""
    from concurrent.futures import ThreadPoolExecutor, as_completed
    ...
    def run_one(ticker: str) -> tuple[str, ScreenerResult | TickerFailure]:
        if circuit_breaker.is_open():
            return ticker, _circuit_open_failure(ticker, endpoint="warm_covered_call_cache")
        rate_limiter.wait()
        ticker_request = request.for_ticker(ticker)
        try:
            task_client = client if client is not None else (client_factory() if client_factory else create_client())
...
def get_best_covered_calls(
    ...
    *,
    ...
    """Find covered calls ranked by yield, risk-adjusted yield, or premium."""
    request = CoveredCallRequest(
        ticker=ticker,
        expiration=expiration,
        min_dte=min_dte,
    ...
    return _execute_covered_call_request(request, client=client, conn=conn).data
