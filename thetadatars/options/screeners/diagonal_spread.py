...
_RIGHT_VALUES = {"call", "put"}
_RANK_BY_VALUES = {"return_if_assigned", "near_credit_to_debit", "net_delta", "debit", "calendar_days"}
...
class DiagonalSpreadRequest:
    ...
    def for_ticker(self, ticker: str) -> "DiagonalSpreadRequest":
        return replace(self, ticker=ticker)


def _request_params(request: DiagonalSpreadRequest) -> dict[str, object]:
    return {
...
def _validate_request(
    ...
    *,
    endpoint: str = "screen_diagonal_spreads",
    ...
    if request.right not in _RIGHT_VALUES:
        raise _invalid_request("right must be one of: call, put", ticker=request.ticker, params=params, endpoint=endpoint)
    ...
    if near_expiration == "*" or far_expiration == "*":
        message = "diagonal spread typed requests with wildcard expirations can fetch broad option chains."
...
def _planned_endpoint_and_params(request: DiagonalSpreadRequest, expiration: dt.date | str) -> tuple[str, dict[str, object]]:
    params = {
...
def _planned_cost(request: DiagonalSpreadRequest, near_expiration: dt.date | str, far_expiration: dt.date | str) -> PlanCost:
...
def _planned_local_computation(request: DiagonalSpreadRequest, near_expiration: dt.date | str, far_expiration: dt.date | str) -> PlanCost:
    if request.greeks_source != "local":
        return "low"
    if (near_expiration == "*" or far_expiration == "*") and request.strike_range is None:
        return "high"
...
def plan_diagonal_spreads(request: DiagonalSpreadRequest, *, conn=None) -> ScreenerPlan:
...
def _build_rows(ticker: str, near_chain: pl.DataFrame, far_chain: pl.DataFrame, *, today: dt.date, right: Right, min_debit: float) -> pl.DataFrame:
    ...
    output = []
    for near_exp, expiration_near_rows in near_by_expiration.items():
        near_rows = [r for r in expiration_near_rows if right_name(r.get("right")) == right]
        for far_exp, expiration_far_rows in far_by_expiration.items():
            if far_exp <= near_exp:
                continue
            far_rows = [r for r in expiration_far_rows if right_name(r.get("right")) == right]
...
def _sort_diagonal_spreads(spreads: pl.DataFrame, *, rank_by: str, top_n: int | None) -> pl.DataFrame:
    tie_breakers = [column for column in ["near_credit_to_debit", "net_delta"] if column != rank_by]
...
def _apply_candidate_caps(spreads: pl.DataFrame, *, rank_by: str, max_candidates_per_expiration: int | None, max_candidates_total: int | None) -> tuple[pl.DataFrame, int]:
    ...
    if max_candidates_per_expiration is not None:
        frames = [
            _sort_diagonal_spreads(group, rank_by=rank_by, top_n=max_candidates_per_expiration)
    ...
    if max_candidates_total is not None:
        capped = _sort_diagonal_spreads(capped, rank_by=rank_by, top_n=max_candidates_total)
    return capped, original_count - len(capped)


def _fetch_chain(request: DiagonalSpreadRequest, *, expiration: dt.date | str, client: Client, today: dt.date, diagnostics: dict[str, object], conn=None) -> pl.DataFrame:
...
def screen_diagonal_spreads(request: DiagonalSpreadRequest, client: Client | None = None, *, conn=None) -> ScreenerResult:
    ...
    try:
        ...
        raise classify_thetadata_error(exc, ticker=ticker, endpoint="screen_diagonal_spreads", params=_request_params(request)) from exc
    fetched_rows = len(near_chain) + len(far_chain)
    filtered_rows = fetched_rows
    diagonals = _build_rows(ticker, near_chain, far_chain, today=today, right=request.right, min_debit=request.min_debit)
    diagonals, pruned_candidate_rows = _apply_candidate_caps(
        diagonals,
        rank_by=request.rank_by,
        max_candidates_per_expiration=request.max_candidates_per_expiration,
        max_candidates_total=request.max_candidates_total,
    )
    candidate_rows = len(diagonals)
    if pruned_candidate_rows:
        warnings = (*warnings, ScreenerWarning(code="candidate_limit", message=f"Candidate limits pruned {pruned_candidate_rows} otherwise eligible diagonal spread candidates."))
    diagonals = _sort_diagonal_spreads(diagonals, rank_by=request.rank_by, top_n=request.top_n)
...
def _attempt_screen(request: DiagonalSpreadRequest, *, client: Client | None, retry_policy: RetryPolicy, timeout_policy: TimeoutPolicy | None = None, conn=None) -> ScreenerResult:
    attempts = 0
    ...
    while attempts < max(retry_policy.max_attempts, 1):
        ...
        try:
            ...
            result = screen_diagonal_spreads(request, client=client, conn=conn)
            if timeout_policy is not None and timeout_policy.per_ticker_seconds is not None and time.perf_counter() - attempt_started > timeout_policy.per_ticker_seconds:
                raise ThetaTimeoutError("Diagonal spread screening exceeded the per-ticker timeout.", ticker=request.ticker, endpoint="screen_diagonal_spreads", params=_request_params(request), retryable=True, user_message="Diagonal spread screening exceeded the per-ticker timeout.")
            return result
        except Exception as exc:
            error = exc if isinstance(exc, ThetaDataError) else classify_thetadata_error(exc, ticker=request.ticker, endpoint="screen_diagonal_spreads", params=_request_params(request))
            last_error = error
...
def screen_diagonal_spread_watchlist(
    tickers: list[str],
    request: DiagonalSpreadRequest,
    ...
    *,
    ...
    def run_one(ticker: str) -> tuple[str, ScreenerResult | TickerFailure]:
        if circuit_breaker.is_open():
            return ticker, _circuit_open_failure(ticker, endpoint="screen_diagonal_spreads")
        rate_limiter.wait()
        ...
        try:
            ...
            return ticker, _failure_for(ticker, exc, attempts=max(retry_policy.max_attempts, 1), endpoint="screen_diagonal_spreads")
...
def _warm_diagonal_spread_request(request: DiagonalSpreadRequest, *, client: Client | None = None, timeout_policy: TimeoutPolicy | None = None, conn=None) -> ScreenerResult:
    started_at = time.perf_counter()
    near_expiration, far_expiration, warnings = _validate_request(request, endpoint="warm_diagonal_spread_cache")
    ticker = request.ticker or ""
    today = dt.date.today()
    plan = plan_diagonal_spreads(request, conn=conn)
    if client is None:
    ...
    if timeout_policy is not None and timeout_policy.per_ticker_seconds is not None and duration > timeout_policy.per_ticker_seconds:
        raise ThetaTimeoutError("Diagonal spread cache warmup exceeded the per-ticker timeout.", ticker=ticker, endpoint="warm_diagonal_spread_cache", params=_request_params(request), retryable=True, user_message="Diagonal spread cache warmup exceeded the per-ticker timeout.")
    fetched_rows = len(near_chain) + len(far_chain)
    return ScreenerResult(
        data=pl.DataFrame(),
...
def _attempt_warm(request: DiagonalSpreadRequest, *, client: Client | None, retry_policy: RetryPolicy, timeout_policy: TimeoutPolicy | None = None, conn=None) -> ScreenerResult:
    attempts = 0
    last_error: ThetaDataError | None = None
    while attempts < max(retry_policy.max_attempts, 1):
        attempts += 1
        try:
            return _warm_diagonal_spread_request(request, client=client, timeout_policy=timeout_policy, conn=conn)
        except Exception as exc:
            error = exc if isinstance(exc, ThetaDataError) else classify_thetadata_error(exc, ticker=request.ticker, endpoint="warm_diagonal_spread_cache", params=_request_params(request))
            last_error = error
            if not error.retryable or attempts >= max(retry_policy.max_attempts, 1) or retry_policy.backoff_seconds <= 0:
                break
            time.sleep(retry_policy.backoff_seconds)
...
def warm_diagonal_spread_cache(
    tickers: list[str],
    request: DiagonalSpreadRequest,
    client: Client | None = None,
    *,
    ...
    def run_one(ticker: str) -> tuple[str, ScreenerResult | TickerFailure]:
        if circuit_breaker.is_open():
            return ticker, _circuit_open_failure(ticker, endpoint="warm_diagonal_spread_cache")
        rate_limiter.wait()
        ticker_request = request.for_ticker(ticker)
        try:
            task_client = client if client is not None else (client_factory() if client_factory else create_client())
            ...
            circuit_breaker.record_failure()
            return ticker, _failure_for(ticker, exc, attempts=max(retry_policy.max_attempts, 1), endpoint="warm_diagonal_spread_cache")
...
def get_best_diagonal_spreads(
    ...
    *,
    ...
    return screen_diagonal_spreads(request, client=client, conn=conn).data
