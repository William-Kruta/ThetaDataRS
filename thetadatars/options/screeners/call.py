...
class CallRequest:
    ...
    def for_ticker(self, ticker: str) -> "CallRequest":
...
def _request_params(request: CallRequest) -> dict[str, object]:
    return {
        ...
        "min_probability_itm": request.min_probability_itm,
        "max_breakeven_move_percent": request.max_breakeven_move_percent,
...
def _planned_cost(request: CallRequest, expiration: dt.date | str) -> PlanCost:
    if expiration == "*" and request.strike_range is None:
        return "high"
...
def _planned_local_computation(request: CallRequest, expiration: dt.date | str) -> PlanCost:
    if request.greeks_source != "local":
        return "low"
    if expiration == "*" and request.strike_range is None:
        return "high"
...
def plan_calls(
    ...
    *,
    ...
    """Explain expected long-call screening cost before fetching data."""
...
def _sort_calls(calls: pl.DataFrame, *, rank_by: str, top_n: int | None) -> pl.DataFrame:
...
def _execute_call_request(
    ...
    *,
    ...
    calls = build_single_leg_options(
        ticker,
        chain,
        option_right="call",
        today=today,
    ...
    calls = _sort_calls(calls, rank_by=request.rank_by, top_n=request.top_n)
...
def screen_calls(
    ...
    *,
    ...
    """Screen long calls from a typed request and return diagnostics."""
    return _execute_call_request(request, client=client, conn=conn)
    ...
    *,
    ...
    while attempts < max(retry_policy.max_attempts, 1):
        ...
        try:
            ...
            result = screen_calls(request, client=client, conn=conn)
            if (
                ...
                raise ThetaTimeoutError(
                    "Call screening exceeded the per-ticker timeout.",
                    ticker=request.ticker,
                    endpoint="screen_calls",
                    params=_request_params(request),
                    retryable=True,
                    user_message="Call screening exceeded the per-ticker timeout.",
                )
            return result
        except Exception as exc:
            error = (
...
def screen_call_watchlist(
    ...
    *,
    ...
    """Screen a watchlist for long calls with per-ticker failures."""
    ...
    def run_one(ticker: str) -> tuple[str, ScreenerResult | TickerFailure]:
        if circuit_breaker.is_open():
            return ticker, _circuit_open_failure(ticker, endpoint="screen_calls")
        rate_limiter.wait()
...
def _warm_call_request(
    ...
    *,
    ...
    started_at = time.perf_counter()
    expiration, warnings = _validate_request(request, endpoint="warm_call_cache")
    ticker = request.ticker or ""
    today = dt.date.today()
    plan = plan_calls(request, conn=conn)
    ...
    if (
        ...
        raise ThetaTimeoutError(
            "Call cache warmup exceeded the per-ticker timeout.",
            ticker=ticker,
            endpoint="warm_call_cache",
            params=_request_params(request),
            retryable=True,
            user_message="Call cache warmup exceeded the per-ticker timeout.",
        )
...
def warm_call_cache(
    ...
    *,
    ...
    """Warm cache inputs for long-call screening without ranking candidates."""
    from concurrent.futures import ThreadPoolExecutor, as_completed
    ...
    def run_one(ticker: str) -> tuple[str, ScreenerResult | TickerFailure]:
        if circuit_breaker.is_open():
            return ticker, _circuit_open_failure(ticker, endpoint="warm_call_cache")
        rate_limiter.wait()
        ticker_request = request.for_ticker(ticker)
        try:
            task_client = client if client is not None else (client_factory() if client_factory else create_client())
...
def get_best_calls(
    ...
    *,
    ...
    """Find long calls ranked by exposure per dollar or other single-leg metrics."""
    request = CallRequest(
        ticker=ticker,
        expiration=expiration,
        min_dte=min_dte,
    ...
    return screen_calls(request, client=client, conn=conn).data


find_best_calls = get_best_calls
