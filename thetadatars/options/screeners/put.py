...
class LongPutRequest:
    ...
    def for_ticker(self, ticker: str) -> "LongPutRequest":
...
def _request_params(request: LongPutRequest) -> dict[str, object]:
    return {
...
def _planned_cost(request: LongPutRequest, expiration: dt.date | str) -> PlanCost:
    if expiration == "*" and request.strike_range is None:
        return "high"
...
def plan_long_puts(
    ...
    *,
    ...
    """Explain expected long-put screening cost before fetching data."""
...
def _execute_long_put_request(
    ...
    *,
    ...
    puts = _sort_long_puts(puts, rank_by=request.rank_by, top_n=request.top_n)
...
def screen_long_puts(
    request: LongPutRequest,
    client: Client | None = None,
    *,
    conn=None,
) -> ScreenerResult:
    """Screen long puts from a typed request and return diagnostics."""
    return _execute_long_put_request(request, client=client, conn=conn)
    ...
    *,
    ...
    while attempts < max(retry_policy.max_attempts, 1):
        ...
        try:
            ...
            result = screen_long_puts(request, client=client, conn=conn)
            if (
                ...
                raise ThetaTimeoutError(
                    ...
                    params=_request_params(request),
                    retryable=True,
                    user_message="Long-put screening exceeded the per-ticker timeout.",
                )
            return result
        except Exception as exc:
            error = (
...
def screen_long_put_watchlist(
    ...
    *,
    ...
    """Screen a watchlist for long puts with per-ticker failures."""
    ...
    def run_one(ticker: str) -> tuple[str, ScreenerResult | TickerFailure]:
        if circuit_breaker.is_open():
            return ticker, _circuit_open_failure(ticker, endpoint="screen_long_puts")
        rate_limiter.wait()
...
def _warm_long_put_request(
    ...
    *,
    ...
    started_at = time.perf_counter()
    expiration, warnings = _validate_request(request, endpoint="warm_long_put_cache")
    ticker = request.ticker or ""
    today = dt.date.today()
    plan = plan_long_puts(request, conn=conn)
    ...
    if (
        ...
        raise ThetaTimeoutError(
            "Long-put cache warmup exceeded the per-ticker timeout.",
            ticker=ticker,
            endpoint="warm_long_put_cache",
            params=_request_params(request),
            retryable=True,
            user_message="Long-put cache warmup exceeded the per-ticker timeout.",
        )
...
def warm_long_put_cache(
    ...
    *,
    ...
    """Warm cache inputs for long-put screening without ranking candidates."""
    from concurrent.futures import ThreadPoolExecutor, as_completed
    ...
    def run_one(ticker: str) -> tuple[str, ScreenerResult | TickerFailure]:
        if circuit_breaker.is_open():
            return ticker, _circuit_open_failure(ticker, endpoint="warm_long_put_cache")
        rate_limiter.wait()
        ticker_request = request.for_ticker(ticker)
        try:
            task_client = client if client is not None else (client_factory() if client_factory else create_client())
...
def get_best_long_puts(
    ...
    *,
    ...
    return screen_long_puts(request, client=client, conn=conn).data
