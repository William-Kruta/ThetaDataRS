...
from ._schemas import OHLCV_COLS

log = logging.getLogger(__name__)
_TABLE = "option_ohlcv"


def fetch_option_history_ohlcv(
    ticker: str,
...
def read_option_history_ohlcv(root: str, start_date: dt.date | str, end_date: dt.date | str | None = None, expiration: dt.date | str | None = None, strike: float | str | None = None, right: Right | None = None, conn=None) -> pl.DataFrame:
    start, end = require_date_range(start_date, end_date)
    expiration = require_expiration(expiration) if expiration is not None else None
    return read_history_table(_TABLE, OHLCV_COLS[:-1], root, start, end, expiration, strike, right, conn=conn, order_columns=["date", "timestamp", "expiration", "strike", "right"])


def get_option_history_ohlcv(
    ...
    strike_range: int | None = None, stale_threshold: dt.timedelta = dt.timedelta(days=7), conn=None,
) -> pl.DataFrame:
    start, end = require_date_range(start_date, end_date)
    expiration = require_expiration(expiration)
    return get_history_table(
        table=_TABLE, columns=OHLCV_COLS,
        fetcher=lambda: fetch_option_history_ohlcv(ticker, start, end, expiration, client, interval, strike, right, start_time, end_time, strike_range),
        log=log, label="option history ohlcv", root=ticker, start_date=start, end_date=end,
