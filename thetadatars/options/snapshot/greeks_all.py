...
_TABLE = "snapshot_greeks_all"
_DB_COLS = ["root", "expiration", "strike", "right", "timestamp", "bid", "ask", "delta", "theta", "vega", "rho", "epsilon", "lambda", "gamma", "vanna", "charm", "vomma", "veta", "speed", "zomma", "color", "ultima", "implied_vol", "iv_error", "underlying_timestamp", "underlying_price", "fetched_at"]
...
def fetch_snapshot_greeks_all(
    ...
    return client.option_snapshot_greeks_all(
        symbol=ticker,
...
def read_snapshot_greeks_all(
    ...
    if own_conn:
        conn = get_connection()
    try:
        q = 'SELECT root, expiration, strike, "right", timestamp, bid, ask, delta, theta, vega, rho, epsilon, "lambda", gamma, vanna, charm, vomma, veta, speed, zomma, color, ultima, implied_vol, iv_error, underlying_timestamp, underlying_price FROM snapshot_greeks_all WHERE root = ?'
        params = [root]
...
def get_snapshot_greeks_all(
    ...
    try:
        ...
        if is_stale:
            ...
            log.info("Fetching all greeks for %s exp=%s from API (%s)", ticker, expiration, reason)
            try:
                df = fetch_snapshot_greeks_all(
                ...
                log.info("Fetched and stored %d all-greeks snapshots for %s", len(df), ticker)
            except Exception:
                log.exception("Failed to fetch all greeks for %s exp=%s", ticker, expiration)
                raise
        else:
            log.debug("Reading all greeks for %s from local DB (fetched_at=%s)", ticker, last_fetched)

        return read_snapshot_greeks_all(ticker, expiration if exp_is_date else None, conn=conn)
    finally:
        if own_conn:
            conn.close()
