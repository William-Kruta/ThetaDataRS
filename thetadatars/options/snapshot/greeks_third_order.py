...
_TABLE = "snapshot_greeks_third_order"
_DB_COLS = ["root", "expiration", "strike", "right", "timestamp", "bid", "ask", "speed", "zomma", "color", "ultima", "implied_vol", "iv_error", "underlying_timestamp", "underlying_price", "fetched_at"]
...
def fetch_snapshot_greeks_third_order(
    ticker: str,
    ...
    return client.option_snapshot_greeks_third_order(
        symbol=ticker,
...
def read_snapshot_greeks_third_order(
    root: str,
    ...
    if own_conn:
        conn = get_connection()
    try:
        q = 'SELECT root, expiration, strike, "right", timestamp, bid, ask, speed, zomma, color, ultima, implied_vol, iv_error, underlying_timestamp, underlying_price FROM snapshot_greeks_third_order WHERE root = ?'
        params = [root]
...
def get_snapshot_greeks_third_order(
    ticker: str,
    expiration: dt.date | str,
    client: Client,
    strike: str = "*",
    ...
    try:
        ...
        if is_stale:
            ...
            log.info("Fetching third-order greeks for %s exp=%s from API (%s)", ticker, expiration, reason)
            try:
                df = fetch_snapshot_greeks_third_order(
                ...
                log.info("Fetched and stored %d third-order greek snapshots for %s", len(df), ticker)
            except Exception:
                log.exception("Failed to fetch third-order greeks for %s exp=%s", ticker, expiration)
                raise
        else:
            log.debug("Reading third-order greeks for %s from local DB (fetched_at=%s)", ticker, last_fetched)

        return read_snapshot_greeks_third_order(ticker, expiration if exp_is_date else None, conn=conn)
    finally:
        if own_conn:
            conn.close()
