...
_TABLE = "snapshot_implied_volatility"
...
def fetch_snapshot_implied_volatility(
    ticker: str,
    ...
    return client.option_snapshot_greeks_implied_volatility(
        symbol=ticker,
...
def read_snapshot_implied_volatility(
    root: str,
    ...
    if own_conn:
        conn = get_connection()
    try:
        q = 'SELECT root, expiration, strike, "right", timestamp, bid, ask, implied_vol, iv_error, underlying_timestamp, underlying_price FROM snapshot_implied_volatility WHERE root = ?'
        params = [root]
...
def get_snapshot_implied_volatility(
    ticker: str,
    expiration: dt.date | str,
    client: Client,
    strike: str = "*",
    ...
    try:
        ...
        if is_stale:
            ...
            log.info("Fetching snapshot IV for %s exp=%s from API (%s)", ticker, expiration, reason)
            try:
                df = fetch_snapshot_implied_volatility(
                ...
                df = df.select([c for c in ["root", "expiration", "strike", "right", "timestamp", "bid", "ask", "implied_vol", "iv_error", "underlying_timestamp", "underlying_price", "fetched_at"] if c in df.columns])
                conn.execute(f"INSERT OR REPLACE INTO {_TABLE} SELECT * FROM df")
                log.info("Fetched and stored %d IV snapshots for %s", len(df), ticker)
            except Exception:
                log.exception("Failed to fetch snapshot IV for %s exp=%s", ticker, expiration)
                raise
        else:
            log.debug("Reading snapshot IV for %s from local DB (fetched_at=%s)", ticker, last_fetched)

        return read_snapshot_implied_volatility(ticker, expiration if exp_is_date else None, conn=conn)
    finally:
        if own_conn:
            conn.close()
